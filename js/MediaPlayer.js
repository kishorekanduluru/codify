...
function AC_Generateobj(objAttrs, params, embedAttrs, location) {
    var str = '';
    if (isIE && isWin && !isOpera) {
        str += '<object ';
...
function AC_FL_RunContent() {
    ...
    AC_Generateobj(ret.objAttrs, ret.params, ret.embedAttrs, ret.location);
}
...
function AC_GetArgs(args, ext, srcParamName, classid, mimeType) {
    ...
    ret.location = new Object();
    for (var i = 0; i < args.length; i = i + 2) {
        var currArg = args[i].toLowerCase();

        switch (currArg) {
            case "location":
                ret.location = args[i + 1];
                break;
            case "classid":
                break;
            case "pluginspage":
