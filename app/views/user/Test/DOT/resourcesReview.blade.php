...
{{ Form::open(array('url' => '/user/test/resourcesSubmit', 'class'=>'form-Div')) }}
	...
	<div id="tabs" >
		...
		@foreach ($FilerDetails as $filer)
		 ...
		 <font class="blockquotehead">
			 <u>{{$filer->IPAddress}}</u><br>
             </font>
		 <div class="display">
			<div class="display1"><font class="blockquote">Mode</font></div>
			<div class="display2">{{$filer->mode}}</div>
			</div>
		 </div>
		 @endforeach
		</div>
		...
		<div id="hostsInfo" class="menu-body" >
			...
			<div class="display2">{{$host->protocol}}</div>
			</div>
