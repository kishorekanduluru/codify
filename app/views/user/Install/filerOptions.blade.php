...
<body onLoad>
<div class="container">
{{ Form::open(array('route' => array('resources.OSinstall.confirm',$resources),'class'=>'form-Div')) }}
	...
	<div id="tabs">
		<ul class="menu-header">
			...
			<li><a href="#{{$filer->id}}" UId="{{$filer->id}}">{{ $filer->ip }}</a></li> 
			@endforeach
			
		</ul>

		@foreach ($filers as $filer)
		<div id="{{$filer->id}}" class="menu-body" >
		...
		<div class="div-mode">
				...
				<h5>Filer Mode</h5>
		<input type="radio" name="mode{{$filer->id}}" value="CMode" id="mode_of{{$filer->id}}" onclick="getModeSelected()" checked="checked">CMode<br>
		<input type="radio" name="mode{{$filer->id}}" value="7Mode" id="mode_of{{$filer->id}}" onclick="getModeSelected()">7Mode<br>
		...
		<div id="SelectionList{{$filer->id}}" class="div-footer">
		<div class="selected_mode" id="selected_mode{{$filer->id}}"  style="display:inline-block"></div>
		</div>
		</div>
...
<script>
	...
	$( "#left_menu" ).empty();
	$("#left_menu").append('<li><a href="/nlab/user/resourceReservation"><span class="tab">Reservation-></span></a></li>');
	$("#left_menu").append('<li><a href="/nlab/user/host/installation"><span class="tab">HostOptions-></span></a></li>');
	$("#left_menu").append('<li class="list"><span class="tab">FilerOptions</span></li>');
